...
func MembershipToUnstructured(r *dclService.Membership) *unstructured.Resource {
	...
	if r.MemberKey != nil && r.MemberKey != dclService.EmptyMembershipMemberKey {
		rMemberKey := make(map[string]interface{})
		if r.MemberKey.Id != nil {
			rMemberKey["id"] = *r.MemberKey.Id
		}
		if r.MemberKey.Namespace != nil {
			rMemberKey["namespace"] = *r.MemberKey.Namespace
		}
		u.Object["memberKey"] = rMemberKey
	}
	if r.Name != nil {
...
func UnstructuredToMembership(u *unstructured.Resource) (*dclService.Membership, error) {
	...
	if _, ok := u.Object["memberKey"]; ok {
		if rMemberKey, ok := u.Object["memberKey"].(map[string]interface{}); ok {
			r.MemberKey = &dclService.MembershipMemberKey{}
			if _, ok := rMemberKey["id"]; ok {
				if s, ok := rMemberKey["id"].(string); ok {
					r.MemberKey.Id = dcl.String(s)
				} else {
					return nil, fmt.Errorf("r.MemberKey.Id: expected string")
				}
			}
			if _, ok := rMemberKey["namespace"]; ok {
				if s, ok := rMemberKey["namespace"].(string); ok {
					r.MemberKey.Namespace = dcl.String(s)
				} else {
					return nil, fmt.Errorf("r.MemberKey.Namespace: expected string")
				}
			}
		} else {
			return nil, fmt.Errorf("r.MemberKey: expected map[string]interface{}")
		}
	}
